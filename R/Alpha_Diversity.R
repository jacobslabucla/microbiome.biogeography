...
#' @param input_data filepath to a csv containing "SampleID", and alpha diversity metrics as columns. "SampleID" must be typed as such.
#' @param input_metadata csv containing "SampleID" and all relevant metadata. Can contain more SampleIDs than are in input_data.
...
#' @examples
#' 
#'generate_adiv_plots("ImmDef-Mouse-Biogeography-Analysis/alpha_diversity_WTCohort.csv", 
#'"ImmDef-Mouse-Biogeography-Analysis/Full-Metadata.csv",
...
generate_adiv_plots <- function(input_data, input_metadata, X, Y, fillvariable, min, max){
  ...
  shannon <- ggplot(data=data,aes(x={{X}},y={{Y}}, fill={{fillvariable}})) + 
